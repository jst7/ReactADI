...
Problema.get('/',autenticaBasic,function(req,res){
  ...
  ultimaPosicion('Problema', function(err, total){
    ...
    var paginas = paginas_paginacion("Problemas/",req ,total)
    connect().query(consulta, function(err, rows, fields) {
    if (err || rows.length==0){
      if(rows.length==0){
        res.status(400).send(Hipermedia('No tiene Problemas',2));
      }else{
        res.status(500).send(Hipermedia('error',2));
      }
    }else{
...
var id = req.params.id;
    ...
    if (err || rows.length==0){
      if(rows.length==0){
        res.status(400).send(Hipermedia('No tiene Problema',2));
      }else{
        res.status(500).send(Hipermedia('error',2));
      }
    }else{
...
var nombre = req.params.nombre;
    ...
    if (err || rows.length==0){
      if(rows.length==0){
        res.status(400).send(Hipermedia('El usuario No tiene Problemas',2));
      }else{
        res.status(500).send(Hipermedia('error',2));
      }
    }else{
