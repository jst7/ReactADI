...
solucion.get('/',autenticaBasic,function(req,res){
  ...
  ultimaPosicion('solucion', function(err, total){
    ...
    var paginas = paginas_paginacion("solucion/",req,total)
    connect().query(consulta, function(err, rows, fields) {
    if (err || rows.length==0){
      if(rows.length==0){
        res.status(400).send(Hipermedia('No tiene Soluciones',4))
      }
      else{
         res.status(500).send(Hipermedia('Error',4))
      }
    }else{
  ...
  connect().query('delete from Soluciones where id = \''+id+'\';', function(err, rows, fields) {
    if (err || rows.affectedRows==0){
      if(rows.affectedRows==0){
            res.status(400).send(Hipermedia('Solucion no Borrado',4));
      }else{
            res.status(500).send(Hipermedia('Error',4));
            }
    }else{
